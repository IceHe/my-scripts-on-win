...
function edit_enex($params){
	...
	if(isset($params["title"]) &&
		NULL === ($c = preg_replace('/<title>.*?<\/title>/', "<title>$params[title]</title>", $c)))	return FALSE;
	...
	if(isset($params["tag"])){
		...
		// echo "$tags\n";	//for debug
		if(NULL === ($c = preg_replace('/<\/updated>.*?<\/note>/', "</updated>$tags</note>", $c)))	return FALSE;
	...
	if(isset($params["created_time"])){
		if("removed" === $params["created_time"]){
			if(NULL === ($c = preg_replace('/<created>.*?<\/created>/', "<created></created>", $c)))	return FALSE;
			...
			if(NULL === ($c = preg_replace('/<created>.*?<\/created>/', "<created>" . gmdate("Ymd\THis\Z", $created_time) . "</created>", $c)))	return FALSE;
	...
	if(isset($params["updated_time"])){
		if("removed" === $params["updated_time"]){
			if(NULL === ($c = preg_replace('/<updated>.*?<\/updated>/', "<updated></updated>", $c)))	return FALSE;
			...
			if(NULL === ($c = preg_replace('/<updated>.*?<\/updated>/', "<updated>" . gmdate("Ymd\THis\Z", $updated_time) . "</updated>", $c)))	return FALSE;
